...
| Model-conditioned IRG binding | `wmloop/geometry/model_irg.py`, `wmloop/control/model_irg_materializer.py`, `configs/schemas/model_irg.schema.json` | Binds a measured response vector to one Model Portrait through a resumable hash-locked CLI; missing responses fail closed instead of being inferred |
| IRG-guided cross-domain discovery | `wmloop/retrieve/coordinator.py`, `wmloop/retrieve/irg_guided_discovery.py`, `wmloop/retrieve/query_policy.py` | Converts supported IRG sensitivity hotspots into policy-versioned multi-view queries and shadow-only mechanism discovery; online modes require a real network evidence receipt by default |
...
| Repair-collision discovery | `wmloop/geometry/evolution.py`, `wmloop/control/probe_evolution_cycle.py` | Nearby IRGs with confidence-separated opposing effects produce portable proposal-only discrimination work orders; successor measurement and admission remain separate |
| Counterexample-driven probe evolution | `wmloop/control/probe_evolution_cycle.py`, `wmloop/experiments/probe_evolution.py`, `scripts/export/probe_evolution.py`, `scripts/export/probe_evolution_settlement.py` | Generic collision planning is wired into the autonomous runner when comparison IRGs are supplied; Cosmos3 scale counterexamples produced and evaluated a temporal-mix successor that correctly settled as abstained |
...
| Online cold-start method discovery | `wmloop/retrieve/providers.py`, `wmloop/retrieve/literature.py`, `wmloop/retrieve/evidence_gate.py`, `wmloop/retrieve/method_staging.py` | Shared bounded arXiv/OpenAlex/GitHub metadata providers feed data-only records; strict synthesis maps known methods to ranking-only evidence and unknown methods to prompt-compatible work orders with no command or GPU authority |
