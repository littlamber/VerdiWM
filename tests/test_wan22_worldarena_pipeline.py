...
from scripts.prepare_wan22_worldarena_input import _bind_evaluator_paths, materialize
...
class Wan22WorldArenaPipelineTests(unittest.TestCase):
    def test_runtime_config_is_bound_structurally(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            assets = root / "assets"
            required = [
                "clip/ViT-B-32.pt", "raft/raft-things.pth", "vfimamba/model.pkl",
                "sea-raft/Tartan-C-T-TSKH-spring540x960-M.pth",
                "dino/dino_vitbase16_pretrain.pth",
            ]
            for name in required:
                path = assets / name; path.parent.mkdir(parents=True, exist_ok=True); path.write_bytes(b"x")
            (assets / "dino" / "facebookresearch_dino").mkdir()
            sea_config = root / "sea.json"; sea_config.write_text("{}")
            config = {}
            _bind_evaluator_paths(config, worldarena_root=worldarena, asset_root=assets, sea_raft_config=sea_config)
            self.assertEqual(config["ckpt"]["motion_smoothness"]["model"], str(assets / "vfimamba" / "model.pkl"))
            self.assertEqual(config["ckpt"]["photometric_smoothness"]["cfg"], str(sea_config))
    ...
    def test_verifier_fails_closed_on_malformed_training_receipt(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            receipt.write_text(json.dumps({
                ...
                "metrics": {metric: {"raw": 0.1} for metric in (
                    "subject_consistency", "background_consistency", "photometric_smoothness"
                )},
            }))
            result = verify([receipt])
            self.assertEqual(result["state"], "blocked")
            self.assertTrue(any("TRAINING_RECEIPT_INVALID" in code for code in result["blockers"]))
    ...
    def test_verifier_requires_same_frozen_panel_for_each_seed(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            metrics = {metric: {"raw": 0.1} for metric in ("subject_consistency", "background_consistency", "photometric_smoothness")}
            for seed in (1, 2):
                seed_root = root / f"seed-{seed}"; seed_root.mkdir()
                (seed_root / "training_receipt.json").write_text(json.dumps({"seed": seed}), encoding="utf-8")
                for sample in ("episode-a:0:150", "episode-b:0:150"):
                    receipt = seed_root / f"metrics-{sample.split(':', 1)[0]}.json"
                    receipt.write_text(json.dumps({
                        "artifact_type": "verdiwm-wan22-droid-worldarena-metrics-receipt",
                        "state": "evaluated_partial", "returncode": 0, "metrics": metrics,
                        "video": {"generated_frames": 150, "fps": 5.0, "sample_id": sample,
