...
class Cosmos3DirectionalSettlementPublicBundleTests(unittest.TestCase):
    def test_exports_final_abstention_without_machine_paths(self) -> None:
        with TemporaryDirectory() as temporary:
                ...
                json.dumps(
                    {
                        ...
                        "terms": {
                            "accept_locality_admitted": True,
                            "dev_accept_jacobian_aligned": False,
                            "dev_locality_admitted": True,
                        },
                        "locality": {
                            "dev_residual": 0.3,
            ...
            self.assertFalse(bundle["certificate_terms"]["dev_accept_jacobian_aligned"])
    ...
    def test_exports_direct_fingerprint_settlement(self) -> None:
        with TemporaryDirectory() as temporary:
            root = Path(temporary)
            dev = root / "dev"
            accept = root / "accept"
                ...
                json.dumps(
                    {
                        "campaign_id": "campaign",
                        "split": "accept",
                        "locality_admission_state": "failed",
                        "videos": [],
                    }
                )
            )
                ...
                json.dumps(
                    {
                        ...
                        "probe_id": "scale",
                        "cross_backbone_transfer_eligible": False,
                        "abstention_reasons": ["accept_locality_admitted"],
                        "terms": {
                            "accept_locality_admitted": False,
                            "dev_accept_jacobian_aligned": False,
                            "dev_locality_admitted": True,
                        },
                        "locality": {
            ...
            self.assertEqual(bundle["dev_source_type"], "direct_frozen_fingerprint")
            self.assertFalse(bundle["certificate_terms"]["accept_locality_admitted"])
            self.assertTrue((root / "public/dev/target-local-fingerprint.json").is_file())
