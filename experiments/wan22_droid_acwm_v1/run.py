...
from scripts.verify_wan22_droid_worldarena import verify as verify_worldarena  # noqa: E402
...
def _budget_snapshot(path: Path | None, total_gpu_hours: float) -> dict[str, float | str | None]:
    if not math.isfinite(total_gpu_hours) or total_gpu_hours <= 0 or total_gpu_hours > 40:
    ...
    if path is not None:
        ...
        if not math.isfinite(consumed) or consumed < 0 or declared_cap != total_gpu_hours:
...
def _runner_command(args: argparse.Namespace, run_root: Path, seed: int, max_gpu_hours: float) -> list[str]:
    command = [
        ...
        "--train-manifest", str(args.train_manifest.expanduser().resolve()),
        "--validation-manifest", str(args.validation_manifest.expanduser().resolve()),
        "--evaluator-contract", str(args.evaluator_contract.expanduser().resolve()),
        "--output-root", str(run_root),
        "--gpu-index", "0" if args.cuda_visible_devices else str(args.gpu_index),
        "--seed", str(seed),
        "--conditioning-mode", args.conditioning_mode,
        "--history-decay", str(args.history_decay),
        "--anchor-policy", args.anchor_policy,
        "--anchor-refresh-strength", str(args.anchor_refresh_strength),
        "--branch-count", str(args.branch_count),
        "--branch-selection", args.branch_selection,
        "--branch-reference-weight", str(args.branch_reference_weight),
        "--sample-index", str(args.sample_index),
        "--horizon-frames", str(args.horizon_frames),
        "--chunk-frames", str(args.chunk_frames),
        "--steps", str(args.steps),
        "--training-stage", "pilot",
        "--training-mode", args.training_mode,
        "--training-sampler", args.training_sampler,
        "--train-record-limit", str(args.train_record_limit),
        "--rollout-steps", str(args.rollout_steps),
        "--learning-rate", str(args.learning_rate),
        "--max-gpu-hours", str(max_gpu_hours),
    ]
    panel = getattr(args, "validation_sample_indices", None)
    if panel:
        command.extend(["--validation-sample-indices", *(str(index) for index in panel)])
    return command
...
def _metrics_receipt_name(dimensions: Sequence[str]) -> str:
    if list(dimensions) == ["subject_consistency", "background_consistency", "photometric_smoothness"]:
        return "worldarena_metrics_receipt.json"
    return "worldarena_metrics_receipt_" + "_".join(dimensions) + ".json"


def _validate_training_receipt(path: Path, args: argparse.Namespace, seed: int) -> dict[str, Any]:
    payload = _read_json(path)
    manifest_count = len(_read_json(args.train_manifest.expanduser().resolve())["records"])
    eligible_windows = manifest_count if args.train_record_limit == 0 else min(manifest_count, args.train_record_limit)
    expected_windows = 1 if args.training_mode == "probe" else min(args.steps, eligible_windows)
    checks = {
        "seed": seed,
    ...
    mismatches = [f"{key}={payload.get(key)!r}:expected={value!r}" for key, value in checks.items() if payload.get(key) != value]
...
def _worldarena_commands(args: argparse.Namespace, run_root: Path, prepared_root: Path) -> tuple[list[str], list[str]]:
    runtime = str(args.worldarena_runtime_python.expanduser().absolute())
    prepare = [
        runtime, str(ROOT / "scripts" / "prepare_wan22_worldarena_input.py"),
        "--run-root", str(run_root), "--output-root", str(prepared_root),
        "--config-template", str(args.worldarena_config_template.expanduser().resolve()),
        "--worldarena-root", str(args.worldarena_root.expanduser().resolve()),
        "--asset-root", str(args.worldarena_asset_root.expanduser().resolve()),
        "--sea-raft-config", str(args.worldarena_sea_raft_config.expanduser().resolve()),
    ]
    evaluate = [
        runtime, str(ROOT / "scripts" / "evaluate_wan22_worldarena.py"),
        "--run-root", str(run_root),
        "--worldarena-root", str(args.worldarena_root.expanduser().resolve()),
        "--config", str(prepared_root / "config.yaml"),
        "--prepared-root", str(prepared_root),
        "--runtime-python", runtime,
        "--dimensions", *args.worldarena_dimensions,
        "--cuda-visible-devices", args.cuda_visible_devices,
        "--asset-manifest", str(args.worldarena_asset_manifest.expanduser().resolve()),
        "--asset-root", str(args.worldarena_asset_root.expanduser().resolve()),
    ]
...
def _closed_loop(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    ...
    elif len(set(args.validation_sample_indices)) != len(args.validation_sample_indices):
        validation_panel_blocker = "VALIDATION_PANEL_INDICES_DUPLICATE"
    elif any(index < 0 or index >= len(validation_records) for index in args.validation_sample_indices):
        validation_panel_blocker = "VALIDATION_PANEL_INDEX_OUT_OF_RANGE"
    else:
        panel_episodes = [str(validation_records[index].get("episode_id", "")).strip() for index in args.validation_sample_indices]
    ...
    worldarena_runtime = args.worldarena_runtime_python.expanduser().absolute() if args.worldarena_runtime_python else runtime
    ...
    estimate = args.estimated_gpu_hours or (
        (_estimated_gpu_hours(args.steps, remaining_training) if remaining_training else 0.0)
    ...
    if training_scale is not None:
        scale_digest = hashlib.sha256(
            json.dumps(training_scale, ensure_ascii=True, sort_keys=True, separators=(",", ":")).encode("utf-8")
        ).hexdigest()
        env.update(
            {
                ...
                "VERDIWM_VALIDATION_PANEL_SIZE": str(len(args.validation_sample_indices)),
            }
        )
    ...
    receipt: dict[str, Any] = {
        ...
        "conformance": report,
        "budget": {**budget, "estimated_gpu_hours": estimate, "run_cap_gpu_hours": args.max_gpu_hours},
        ...
        "stages": ["train", "rollout_150f", "worldarena_frozen_eval", "multi_seed_verification", "promotion_decision"],
    ...
    for seed in args.seeds:
        ...
        run_root = resumed[0] if resumed else _next_attempt_path(output / f"seed-{seed}")
        if resumed is None:
            run_root.mkdir(mode=0o700, parents=True, exist_ok=False)
        elapsed_hours = (time.monotonic() - started) / 3600.0
        available_hours = min(args.max_gpu_hours - resume_spent - elapsed_hours, remaining - elapsed_hours)
        row: dict[str, Any] = {
            "seed": seed, "state": "running", "run_root": str(run_root),
        ...
        try:
            ...
            if resumed is None:
                ...
                if free_mib < args.min_free_gpu_memory_mib:
                    raise RuntimeError(f"GPU_FREE_MEMORY_BELOW_TRAINING_FLOOR:{free_mib}")
                runner_command = _runner_command(args, run_root, seed, available_hours)
                row["runner_command"] = runner_command
                completed = _run_logged(
                    runner_command, cwd=args.runner.expanduser().resolve().parent, env=env,
                    stdout_path=run_root / "runner.stdout.log", stderr_path=run_root / "runner.stderr.log",
                    timeout_seconds=available_hours * 3600.0,
                )
                if completed.returncode != 0:
                    raise RuntimeError(f"RUNNER_FAILED:{completed.returncode}")
                training = _validate_training_receipt(run_root / "training_receipt.json", args, seed)
            else:
                training = resumed[1]
            ...
            if panel_manifest.get("state") != "frozen" or not isinstance(panel_rows, list):
                raise ValueError("VALIDATION_PANEL_RECEIPT_INVALID")
            if [int(item.get("sample_index", -1)) for item in panel_rows if isinstance(item, dict)] != list(args.validation_sample_indices):
            ...
            for panel_item in panel_rows:
                ...
                panel_run_root = Path(str(panel_item["run_root"])).expanduser().resolve(strict=True)
                sample_index = int(panel_item["sample_index"])
                prepared_root = _next_attempt_path(output / "worldarena" / f"seed-{seed}" / f"panel-{sample_index}")
                free_mib = _gpu_free_memory_mib(args.cuda_visible_devices)
                row.setdefault("free_gpu_memory_before_evaluation_mib", free_mib)
                if free_mib < args.min_free_evaluator_gpu_memory_mib:
                    raise RuntimeError(f"GPU_FREE_MEMORY_BELOW_EVALUATOR_FLOOR:{free_mib}")
                prepare_command, evaluate_command = _worldarena_commands(args, panel_run_root, prepared_root)
                prepared_root.parent.mkdir(mode=0o700, parents=True, exist_ok=True)
                prepared = _run_logged(
                    prepare_command, cwd=ROOT, env=env,
                    stdout_path=panel_run_root / "worldarena_prepare.stdout.log",
                    stderr_path=panel_run_root / "worldarena_prepare.stderr.log",
                    timeout_seconds=max(1.0, (args.max_gpu_hours - (time.monotonic() - started) / 3600.0) * 3600.0),
                )
                if prepared.returncode != 0:
                    raise RuntimeError(f"WORLDARENA_PREPARE_FAILED:{prepared.returncode}:panel={sample_index}")
                evaluated = _run_logged(
                    evaluate_command, cwd=ROOT, env=env,
                    stdout_path=panel_run_root / "worldarena_adapter.stdout.log",
                    stderr_path=panel_run_root / "worldarena_adapter.stderr.log",
                    timeout_seconds=max(1.0, (args.max_gpu_hours - (time.monotonic() - started) / 3600.0) * 3600.0),
                )
                if evaluated.returncode != 0:
                    raise RuntimeError(f"WORLDARENA_EVALUATION_FAILED:{evaluated.returncode}:panel={sample_index}")
                metrics_path = panel_run_root / _metrics_receipt_name(args.worldarena_dimensions)
                metrics = _read_json(metrics_path)
                if metrics.get("state") != "evaluated_partial" or metrics.get("returncode") != 0:
    ...
    receipt["budget"]["remaining_after_upper_bound_gpu_hours"] = float(budget["remaining_before_gpu_hours"]) - actual_hours
    if failure is not None:
        receipt.update({"state": "failed", "failure": failure, "next_state": "capability_gap_triage"})
        _write_json(receipt_path, receipt)
        return receipt, 2
    ...
    if verification["state"] != "verified":
        receipt.update({"state": "failed", "blockers": verification["blockers"], "next_state": "capability_gap_triage"})
        code = 2
    else:
        receipt.update({
            "state": "completed",
...
def _parser() -> argparse.ArgumentParser:
    ...
    closed = commands.add_parser("closed-loop", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    closed.add_argument("--min-free-evaluator-gpu-memory-mib", type=float, default=8000.0)
    closed.add_argument("--execute", action="store_true", help="allow the bound runner to use GPU")
    closed.add_argument("--resume", action="store_true", help="reuse matching completed seed training receipts and preserve failed attempts")
    ...
    closed.add_argument("--training-sampler", choices=("sequential", "episode_balanced"), default="episode_balanced")
    closed.add_argument("--train-record-limit", type=int, default=256, help="bounded long-training windows; 0 makes the full manifest eligible within the global step count")
    closed.add_argument("--steps", type=int, default=512, help="global optimization updates, not updates per window")
    closed.add_argument("--seeds", type=int, nargs="+", default=list(DEFAULT_SEEDS))
    closed.add_argument("--sample-index", type=int, default=0)
    closed.add_argument("--validation-sample-indices", type=int, nargs="+", default=None,
                        help="frozen validation panel indices; formal runs require three distinct episodes")
    closed.add_argument("--conditioning-mode", choices=("visual_anchor_only", "action", "action_proprio", "action_proprio_history", "action_proprio_ema"), default="action_proprio_ema")
    closed.add_argument("--history-decay", type=float, default=0.8)
    closed.add_argument("--anchor-policy", choices=("previous_generated", "initial_reference_blend"), default="initial_reference_blend")
    closed.add_argument("--anchor-refresh-strength", type=float, default=0.25)
    closed.add_argument("--branch-count", type=int, default=1)
    closed.add_argument("--branch-selection", choices=("first", "terminal_reference_consistency"), default="first")
    ...
    closed.add_argument("--max-gpu-hours", type=float, default=15.0, help="hard wall-clock GPU-hour upper bound for this serial batch")
    ...
    closed.add_argument("--worldarena-sea-raft-config", type=Path, default=ROOT / "configs" / "evaluators" / "sea_raft_192x320.json")
    closed.add_argument("--worldarena-dimensions", nargs="+", default=list(DEFAULT_DIMENSIONS))
    return parser
