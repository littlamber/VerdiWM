...
_CAPS = {"very_low": 0.5, "low": 8.0, "medium": 48.0, "high": 120.0}
...
class BudgetLedger:
    ...
    def admit(
        ...
        *,
        ...
        _validate_admission(trial_id, cost_class, estimated_gpu_hours, human_approved)
...
def _validate_admission(trial_id: str, cost_class: str, estimated_gpu_hours: float, human_approved: bool) -> None:
    if not trial_id or cost_class not in _CAPS or not math.isfinite(estimated_gpu_hours) or estimated_gpu_hours <= 0:
        raise BudgetError("TRIAL_ADMISSION_INVALID")
    if estimated_gpu_hours > _CAPS[cost_class]:
        raise BudgetError("TRIAL_COST_CAP_EXCEEDED")
    if cost_class == "high" and not human_approved:
