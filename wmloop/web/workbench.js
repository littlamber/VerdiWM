...
async function boot() { try { const [modes, project] = await Promise.all([api("/api/modes"), api("/api/project"), refreshCampaigns()]); state.modes = modes.items; renderModes(); bindGraph(); const values = project.values || {}; ["model", "dataset", "goal", "budget", "adapter", "target_metrics"].forEach((key) => { if (values[key] && $( `[name="${key}"]`)) $( `[name="${key}"]`).value = Array.isArray(values[key]) ? values[key].join(" ") : values[key]; }); $("#project-state").textContent = project.configured ? "已加载项目配置；默认值由系统内部管理。" : "未发现 verdiwm.toml；可在高级设置中提供项目路径。"; $("#api-state").textContent = "本地服务已连接"; $("#api-state").classList.add("ready"); } catch (error) { $("#api-state").textContent = error.message; } }
boot();
