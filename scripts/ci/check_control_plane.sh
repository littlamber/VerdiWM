...
python -c 'import sys, zipfile; names=set(zipfile.ZipFile(sys.argv[1]).namelist()); assert "wmloop/control/model_run.py" in names; assert "wmloop/execute/model_runner.py" in names; assert "wmloop/execute/model_adapters.py" in names; assert "configs/schemas/model_run_manifest.schema.json" in names' "$wheel_path"
python -c 'import sys, zipfile; names=set(zipfile.ZipFile(sys.argv[1]).namelist()); assert "scripts/run_ctrl_world_model_train.py" in names; assert "scripts/run_ctrl_world_model_eval.py" in names; assert "scripts/evaluate_ctrl_world_acwm_dual.py" in names; assert "configs/experiments/ctrl_world_acwm_dual_evaluation_v1.json" in names' "$wheel_path"
